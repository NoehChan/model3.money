      ...
      new route53.ARecord(this, `route53-a-record${counter}`, {
        ...
        target: route53.RecordTarget.fromAlias(
          new targets.CloudFrontTarget(
            cloudfrontToS3Resource.cloudFrontWebDistribution
          )
        ),
        recordName: domain,
      });
  ...
  constructor(scope: Construct, id: string, props?: StageProps) {
    ...
    new Model3DotMoneyStack(this, 'model3-money', {
      route53_HostedZoneId:
        process.env.route53_HostedZoneId ??
        this.node.tryGetContext('route53_HostedZoneId'),
      route53_ZoneName:
        process.env.route53_ZoneName ??
        this.node.tryGetContext('route53_ZoneName'),
      domains: String(
        process.env.domains ?? this.node.tryGetContext('domains')
      ).split(','),
    });
  }
}
